...
const GithubRepoLink = ({ repo, className }: GithubRepoLinkProps) => {
  const [stars, setStars] = React.useState<number | null>();

  React.useEffect(() => {
    async function fetchStars() {
      console.log("fetch those stars!");
      const response = await fetch(
        `https://api.github.com/repos/noahgorstein/${repo}`
      );
  ...
  return (
    <a
      ...
      href={`https://github.com/noahgorstein/${repo}`}
      target="_blank"
    >
