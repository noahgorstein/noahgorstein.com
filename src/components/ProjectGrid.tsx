...
const ProjectGrid: React.FC<ProjectGridProps> = ({ projects }: ProjectGridProps) => {
  ...
  return (
    <section className="grid grid-cols-12">
      ...
      <ul
        className="col-span-12 mt-4 flex flex-wrap items-stretch justify-between gap-8 md:col-span-8 md:grid-cols-3"
      >
        {
          projectData.map((project) => (
            <li key={project.title} className="group flex basis-full flex-col gap-8 rounded-xl border-2 border-foreground px-4 py-4 transition-colors hover:border-emerald md:col-span-1 md:basis-[calc(50%-3rem)] lg:basis-[calc(33.33%-3rem)]">
              <a
